...
import { ChangePassword, ForgotPasswordService } from 'src/app/services/forgot-password.service';
...
export class ForgotPasswordComponent {
  ...
  verifyOtp() {
    if(this.verifyOtpForm.valid) {
      ...
      this.forgotPasswordService.verifyOtpService(this.verifyOtpForm.get('otp')?.value, this.savedEmail).subscribe({
        next: (response) => {
          this.verifiedOtp = this.verifyOtpForm.get('otp')?.value;
          this.isLoading = false;
          console.log(response);
          this.state2 = false;
          this.state3 = true;
  ...
  changePassword() {
    if(this.changePasswordForm.valid) {
      ...
      this.isLoading = true;
      this.forgotPasswordService.changePasswordService(changePassword, this.savedEmail, this.verifiedOtp).subscribe({
        next:  (response) => {
          this.isLoading = false;
          console.log(response);
